import { useEffect, useRef, useState } from "react";
import { CheckCircle2 } from "lucide-react";
import { useNavigate, useSearchParams } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import {
  confirmServiceRenew,
  confirmServicePurchase,
  selectServicePurchaseConfirmError,
  selectServicePurchaseConfirmLoading,
  selectServiceRenewConfirmError,
  selectServiceRenewConfirmLoading,
} from "../../../features/services/servicesSlice";

export default function ServicePurchaseSuccess() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const [searchParams] = useSearchParams();
  const purchaseConfirmLoading = useSelector(selectServicePurchaseConfirmLoading);
  const purchaseConfirmError = useSelector(selectServicePurchaseConfirmError);
  const renewConfirmLoading = useSelector(selectServiceRenewConfirmLoading);
  const renewConfirmError = useSelector(selectServiceRenewConfirmError);
  const hasConfirmedRef = useRef(false);
  const [selectedPlanId, setSelectedPlanId] = useState("");
  const [checkoutSessionId, setCheckoutSessionId] = useState("");
  const [hasConfirmedSuccess, setHasConfirmedSuccess] = useState(false);

  const isValidCheckoutSessionId = (value) => {
    const raw = String(value || "").trim();
    if (!raw) return false;
    if (raw.includes("CHECKOUT_SESSION_ID") || raw.includes("{") || raw.includes("}")) return false;
    return true; // PayPal order IDs don't have a specific prefix like Stripe's 'cs_'
  };

  const serviceId = searchParams.get("serviceId") || "";
  const flow = (searchParams.get("flow") || "purchase").toLowerCase();
  const isRenewFlow = flow === "renew";
  const initialPlanId =
    searchParams.get("planId") ||
    searchParams.get("pricingPlanId") ||
    "";
  const initialCheckoutSessionId =
    searchParams.get("token") ||
    searchParams.get("session_id") ||
    searchParams.get("checkoutSessionId") ||
    searchParams.get("sessionId") ||
    "";
  const savedContextRaw =
    sessionStorage.getItem("servicePaymentConfirmContext") ||
    sessionStorage.getItem("servicePurchaseConfirmContext");
  let savedContext = null;

  try {
    savedContext = savedContextRaw ? JSON.parse(savedContextRaw) : null;
  } catch {
    savedContext = null;
  }

  const sameServiceContext = String(savedContext?.serviceId || "") === String(serviceId || "");
  const resolvedPlanId = initialPlanId || (sameServiceContext ? String(savedContext?.planId || "") : "");
  const hasUsableInitialSessionId = isValidCheckoutSessionId(initialCheckoutSessionId);
  const resolvedCheckoutSessionId =
    (hasUsableInitialSessionId ? initialCheckoutSessionId : "") ||
    (sameServiceContext ? String(savedContext?.checkoutSessionId || "") : "");

  const missingParams = !hasConfirmedSuccess && (!serviceId || !resolvedPlanId);
  const callbackLoading = isRenewFlow ? renewConfirmLoading : purchaseConfirmLoading;
  const callbackError = isRenewFlow ? renewConfirmError : purchaseConfirmError;

  useEffect(() => {
    setSelectedPlanId(resolvedPlanId);
    setCheckoutSessionId(resolvedCheckoutSessionId);
  }, [resolvedCheckoutSessionId, resolvedPlanId]);

  useEffect(() => {
    if (missingParams || hasConfirmedRef.current || !selectedPlanId || !checkoutSessionId) return;

    hasConfirmedRef.current = true;

    const confirmThunk = isRenewFlow ? confirmServiceRenew : confirmServicePurchase;

    dispatch(
      confirmThunk({
        serviceId,
        payload: {
          planId: selectedPlanId,
          checkoutSessionId,
        },
      })
    )
      .unwrap()
      .then((response) => {
        if (response?.success) {
          setHasConfirmedSuccess(true);
          setSelectedPlanId("");
          setCheckoutSessionId("");
          sessionStorage.removeItem("servicePaymentConfirmContext");
          sessionStorage.removeItem("servicePurchaseConfirmContext");
        }
      })
      .catch(() => {
        // Error UI is rendered from redux state.
      });
  }, [checkoutSessionId, dispatch, isRenewFlow, missingParams, selectedPlanId, serviceId]);

  if (missingParams) {
    return (
      <div className="min-h-screen bg-[#f3f3f3] flex items-center justify-center px-4">
        <div className="text-center max-w-lg w-full">
          <h1 className="text-3xl font-bold text-gray-900 mb-3">Invalid Payment Callback</h1>
          <p className="text-gray-600 mb-6">Missing serviceId or planId in callback URL.</p>
          <button
            onClick={() => navigate("/profile/my-services")}
            className="px-6 py-2.5 rounded bg-[#E97C35] text-white font-semibold"
          >
            Go To Services
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#f3f3f3] flex items-center justify-center px-4">
      <div className="text-center max-w-lg w-full">
        <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-[#d8f5ea]">
          <CheckCircle2 size={34} className="text-[#22b581]" />
        </div>
        <h1 className="text-5xl font-bold text-black mb-4">Payment Successful</h1>
        <p className="text-2xl text-gray-600 leading-relaxed mb-8">
          {isRenewFlow
            ? "Your renewal payment has been successfully processed. Your listing renewal is now submitted for admin review."
            : "Your payment has been successfully processed. Your listing has been submitted and will appear on the platform once it is approved."}
        </p>

        {callbackLoading && (
          <p className="text-sm text-[#6b7280] mb-4">Finalizing your purchase confirmation...</p>
        )}

        {!callbackLoading && callbackError && (
          <p className="text-sm text-red-600 mb-4">{callbackError}</p>
        )}

        <button
          onClick={() => navigate("/profile/my-services")}
          className="inline-flex items-center justify-center px-8 py-3 rounded bg-[#E97C35] text-white font-semibold hover:bg-[#d06b2f] transition-colors"
        >
          View Listing
        </button>
      </div>
    </div>
  );
}
